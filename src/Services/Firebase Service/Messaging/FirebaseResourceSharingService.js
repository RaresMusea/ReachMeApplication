...
const sendMediaMessage = async (payload, messageConfiguration, type, path) => {
  ...
  const uploadTask = storage
    .ref(`Conversations/${messageConfiguration.conversationId}`)
    .child(path)
    .child(fileName)
    .put(payload);
  ...
  uploadTask.on(
    ...
    (snapshot) => {
      ...
      displayUploadSnackBar(`Sending ${
        messageType === null ? type : messageType
      } 
...
const sendMultipleMediaMessages = async (
  payload,
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          uploadedPhotoUrl = downloadURL;
          sendMessage(
            type === "file" ? type + "/" + getFileExtension(file) : type,
            uploadCount === 0 &&
              !isEmptyString(messageConfiguration.messageContent)
              ? messageConfiguration.messageContent
              : "",
          ...
          displayUploadSnackBar(`Sending ${payload.length} ${type}s to ${
            messageConfiguration.receiver.userRealName
          }.
     Overall progress: ${uploadCount + 1} out of ${
            payload.length
          } ${type}s sent 
...
export const sendFileOrDocument = async (
  payload,
  messageConfiguration,
  type
) => {
